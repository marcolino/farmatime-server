/**
 * Stripe payment controller implementation for a SAS (always single product, no carts, no delivery...)
 */
      ...
      line_items: [
        {
          ...
          price: config.payment.stripe.products[product]?.price_id, // TODO: create price_id dynamically (or from product...)
          quantity: 1,
        },
      ],
      mode: "payment",
...
const paymentSuccess = async(req, res) => {
  try {
    ...
    res.redirect(config.payment.stripe.paymentSuccessUrlClient + `?product=&error=${err.message ?? err.raw?.message}`);
  }
};
...
const paymentCancel = async(req, res) => {
  try {
    ...
    res.redirect(config.payment.stripe.paymentCancelUrlClient + `?product=&error=${err.message ?? err.raw?.message}`);
  }
};
