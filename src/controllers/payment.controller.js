...
const paymentSuccess = async (req, res, next) => {
  ...
  try {
    ...
    audit({req, mode: "error", subject: `Error retrieving payment info on payment success`, htmlContent: `Payment checkout session creation error for customer ${customer?.name || "unknown"} (email: ${customer?.email || "unknown"}):\n${err.message}` });
...
const paymentCancel = async (req, res, next) => {
  ...
  try {
    const session = await stripe.checkout.sessions.retrieve(req.parameters.session_id);
    customer = session.customer ? await stripe.customers.retrieve(session.customer) : null;
    //const items = await stripe.checkout.sessions.listLineItems(session.id);
    ...
    audit({req, mode: "error", subject: `Error retrieving payment info on payment cancel`, htmlContent: `Payment checkout session creation error for user ${customer?.name || "unknown"} (email: ${customer?.email || "unknown"}):\n${err.message}` });
